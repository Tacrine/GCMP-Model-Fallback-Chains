...
本文档是 `copilot-model-fallback`（GCMP Companion: Model Fallback Chains）的完整配置字段参考。所有键均属于 `contributes.configuration`，在 VS Code 设置中以 `fallbackRouter.*` 前缀出现；默认值来自 `package.json` 与 `src/config.ts` 中的 `normalizeConfig`。
