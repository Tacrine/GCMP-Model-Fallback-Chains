...
function httpTarget(model = 'm', toolCalling = true): Target {
  return { kind: 'http', baseUrl: 'https://t/v1', apiType: 'chat-completions', model, secretRef: 'r', toolCalling };
}
...
function runOptions(overrides: Record<string, unknown> = {}) {
  return {
    ...
    timeouts: { connectMs: 100, headersMs: 100, firstByteMs: 100, stallMs: 1000 },
    ...overrides,
  };
}
...
describe('FallbackRouter', () => {
  it('succeeds on first target without switching', async () => {
    ...
    const res = await router.run({ id: 'a', name: 'a', targets: [httpTarget('m1'), httpTarget('m2')] }, langMsg('q'), runOptions(), emit, cancelToken());
  ...
  it('retries retryable failure to maxAttempts then switches to next target', async () => {
    ...
    const router = newRouter({ transports: (t) => (t.model === 'm1' ? transportA : transportB) });
    const res = await router.run({ id: 'a', name: 'a', targets: [httpTarget('m1'), httpTarget('m2')] }, langMsg('q'), runOptions({ retry: { maxAttempts: 3, initialDelayMs: 1, backoffFactor: 2, maxDelayMs: 5 } }), emit, cancelToken());
  ...
  it('throws aggregated error when all targets fail', async () => {
    ...
    const router = newRouter({ transports: () => transport });
    await expect(router.run({ id: 'a', name: 'a', targets: [httpTarget('m1'), httpTarget('m2')] }, langMsg('q'), runOptions({ retry: { maxAttempts: 1, initialDelayMs: 1, backoffFactor: 2, maxDelayMs: 5 } }), vi.fn(), cancelToken())).rejects.toBeInstanceOf(RouterError);
  });
  ...
  it('treats NoPermissions as non-retryable but still switches target', async () => {
    ...
    const router = newRouter({ transports: (t) => (t.model === 'm1' ? transportA : transportB) });
    const res = await router.run({ id: 'a', name: 'a', targets: [httpTarget('m1'), httpTarget('m2')] }, langMsg('q'), runOptions({ retry: { maxAttempts: 5, initialDelayMs: 1, backoffFactor: 2, maxDelayMs: 5 } }), vi.fn(), cancelToken());
  ...
  it('isolates circuit breakers per target', async () => {
    ...
    const router = newRouter({ transports: (t) => (t.model === 'm1' ? transportA : transportB) });
    await expect(router.run({ id: 'a', name: 'a', targets: [httpTarget('m1')] }, langMsg('q'), runOptions({ retry: { maxAttempts: 1, initialDelayMs: 1, backoffFactor: 2, maxDelayMs: 5 } }), vi.fn(), cancelToken())).rejects.toBeInstanceOf(RouterError);
    // A's breaker now open; B still usable
    const router2 = newRouter({ transports: (t) => (t.model === 'm1' ? transportA : transportB) });
    const res = await router2.run({ id: 'a', name: 'a', targets: [httpTarget('m2')] }, langMsg('q'), runOptions(), vi.fn(), cancelToken());
    expect(res.ok).toBe(true);
  });
  ...
  it('skips target whose circuit breaker is open', async () => {
    ...
    // pre-open breaker for m1
    const key = 'http:https://t/v1/m1';
    ...
    const res = await r.run({ id: 'a', name: 'a', targets: [httpTarget('m1')] }, langMsg('q'), runOptions({ retry: { maxAttempts: 1, initialDelayMs: 1, backoffFactor: 2, maxDelayMs: 5 } }), vi.fn(), cancelToken());
    expect(res.ok).toBe(false);
    expect(called).not.toHaveBeenCalled();
    expect(res.attempts[0].skippedReason).toBe('circuit open');
  });

  it('preflight-skips targets with insufficient maxInputTokens', async () => {
    const called = vi.fn();
    const transport = makeTransport(async () => { called(); return { ok: true, emittedParts: 0, emittedToolCall: false }; });
    const router = newRouter({ transports: () => transport });
    const target: Target = { kind: 'http', baseUrl: 'https://t/v1', apiType: 'chat-completions', model: 'tiny', secretRef: 'r', maxInputTokens: 1 };
    const res = await router.run({ id: 'a', name: 'a', targets: [target] }, langMsg('a very very long message that surely exceeds one token'), runOptions(), vi.fn(), cancelToken());
    expect(res.ok).toBe(false);
    expect(called).not.toHaveBeenCalled();
    expect(res.attempts[0].skippedReason).toBeTruthy();
  });

  it('preflight-skips targets without tool calling when tools requested', async () => {
    const called = vi.fn();
    const transport = makeTransport(async () => { called(); return { ok: true, emittedParts: 0, emittedToolCall: false }; });
    const router = newRouter({ transports: () => transport });
    const opts = runOptions();
    opts.tools = [{ name: 'read', description: '', inputSchema: {} }];
    const res = await router.run({ id: 'a', name: 'a', targets: [httpTarget('m1', false)] }, langMsg('q'), opts, vi.fn(), cancelToken());
    expect(called).not.toHaveBeenCalled();
    expect(res.attempts[0].skippedReason).toBeTruthy();
  });
  ...
  it('respects maxTargetsPerTurn truncation', async () => {
    const calls: string[] = [];
    const transport = makeTransport(async (_t) => { calls.push((_t as { model: string }).model); return { ok: false, error: new Error('fail'), retryable: false, emittedParts: 0, emittedToolCall: false }; });
    const router = newRouter({ transports: () => transport });
    const targets = [httpTarget('m1'), httpTarget('m2'), httpTarget('m3'), httpTarget('m4')];
  ...
  it('stops on cancellation without trying next target', async () => {
    ...
    await expect(router.run({ id: 'a', name: 'a', targets: [httpTarget('m1'), httpTarget('m2')] }, langMsg('q'), runOptions({ retry: { maxAttempts: 1, initialDelayMs: 1, backoffFactor: 2, maxDelayMs: 5 } }), vi.fn(), token)).rejects.toBeInstanceOf(CancellationError);
    expect(called).not.toHaveBeenCalled();
  });
});
